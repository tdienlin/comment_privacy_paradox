...
The results equal those reported in the paper (with the exception that in the paper RMSEA is falsely reported as .008, which seems to be a typo). 
...
We then configure a new preferred model.

![](figures/model_preferred.png)
...
model_preferred <- "
  ...
  effect_pri_ris := (d*f) + e
"
fit_preferred <- lavaan::sem(model_preferred, sample.cov = cor_ma, sample.nobs = n_hm)
```

We extract the results.

```{r}
lavaan::summary(fit_preferred, standardized = TRUE, header = FALSE, ci = TRUE)
```
...
![](figures/model_preferred_results.png)
