...
In what follows, we reanalyse the study by Yu et al. (2020).
...
Referring to Problem 1, we first calculate our so-called `model_ideal`.

```{r}
model_ideal <- "
  pri_ris ~ a * pri_con 
  dis_int ~ b * pri_con + d * pri_ris
  dis_beh ~ c * pri_con + e * pri_ris + f * dis_int
  indirect_effects := (a*d*f) + (a*e) + (b*f)
  direct_effect := c
  total_effect := indirect_effects + direct_effect
"
fit_ideal <- lavaan::sem(model_ideal, sample.cov = cor_ma, sample.nobs = n_hm)
```

We then extract the results.

```{r}
lavaan::summary(fit_ideal, standardized = TRUE, header = FALSE)
```

The results show that the total effect equals the sum of the indirect effects and the direct effects. The total effect also equals the bivariate correlation reported above.

# Model Reported in Paper

Referring to Problem 2, we rebuild the model reported in the paper, called `model_reported`.
...
```{r}
fit_indices <- c("chisq", "df", "pvalue", "cfi", "nfi", "rmsea", "rmsea.ci.lower", "rmsea.ci.upper", "srmr")
lavaan::fitMeasures(fit_reported, fit_indices)
```

The results equal those reported in the paper (with the exception that in the paper RMSEA is falsely reported as .008, which seems to be a typo). Also note that there is one degree of freedom, because the path `pri_con` on `dis_beh` is not included.
...
# Analysis of Saturated Model

We now estimate the saturated model (`model_saturated`) by adding `pri_con` as predictor for `dis_beh`. 
...
```{r}
lavaan::summary(fit_saturated, standardized = TRUE, header = FALSE)
```
